...
corp_tax_raw <-  read_dta(paste0(here(),int_data_path, "/final/corp_tax_raw.dta"))
...
# write_rds(before_data, paste0(here(),output_folder, "/shiny_data/before.rds"))
...
#baseline_data <- readRDS("~/Dropbox/PhD thesis/Internship World Bank/CEQ - Dashboard/CEQ - assessement tool/output/shiny_data/baseline_data.rds")
...
baseline_data <- baseline_data %>%
  group_by(hhid) %>%
  mutate(hhsize = first(hhsize[!is.na(hhsize)])) %>%
  ungroup()
...
baseline_data <- baseline_data %>%
  select(hhid, pid, ea_id, region, district, reside, 
         ...
         hh_n21a, hh_n21b, hh_n15b, hh_n40, tax_rate,
         industry_sectors, i_dtx_payt_hh, tax,
...
# Health and education expenditures by household
baseline_data <- baseline_data %>%
  mutate(across(c(dtx_PIT_hh, dtx_payt_hh, dct_hh, dtr_nct_hh, yd_hh,
                  sub_electri_hh, sub_fuel_hh, itx_vatx_hh, itx_excx_hh,
                  educ_hh, health_hh, Users_fee_hh), 
...
# baseline_data <- baseline_data %>%
#   mutate(
#     dtx_all_hh = round(dtx_all_hh),
#     dtr_all_hh = round(dtr_all_hh),
#     itx_all_hh = round(itx_all_hh),
#     sub_all_hh = round(sub_all_hh),
#     educ_hh = round(educ_hh),
#     health_hh = round(health_hh),
#     Users_fee_hh = round(Users_fee_hh)
#   )
...
baseline_data <- baseline_data %>% mutate(across(where(is.numeric), ~ round(.x, 10)))
