                                ...
                                accordion(
                                  ...
                                  accordion_panel(
                                    ...
                                    div(id = ns("incid_dtx_line_wrapper"), 
                                        ...
                                        radioButtons(inputId = ns("incid_dtx_type"), label = "Taxes: ", choices = c("All Direct Taxes", "PAYE Income Tax","Corporate Income Tax"), selected = "All Direct Taxes"),
                                        radioButtons(inputId = ns("incid_dtx_parameter"), label = "Parameter: ", choices = c("Absolute incidence", "Relative incidence","Total level"), selected = "Absolute incidence")
...
incid_dtx_mod_server <- function(id, simulated_df, root_session) {
  moduleServer(id, function(input, output, session) {
    ...
    incid_data <- reactive({
      ...
      tax_var <- tax_map[[ input$incid_dtx_type ]] %||% "dtx_all_hh"
      ...
      temp_data <- make_weighted_deciles(temp_data, "input_income", as.numeric(input$incid_dtx_nbr_dec))
    ...
    output$incid_dtx_title <- renderUI({
      ...
      div(
        ...
        tags$h6(paste0("Taxes: ", input$incid_dtx_type)), # selected Area
    ...
    output$incid_dtx_chart <- renderHighchart({
      ...
      param_col <- dplyr::case_when(
        input$incid_dtx_parameter == "Relative incidence" ~ "relative_incidence_pct",
        input$incid_dtx_parameter == "Total level"        ~ "level_tax",
    ...
    output$incid_dtx_table <- renderReactable({
      ...
      value_col_name <- case_when(
        input$incid_dtx_parameter == "Absolute incidence" ~ "Absolute incidence (%)",
        input$incid_dtx_parameter == "Relative incidence" ~ "Relative incidence (%)",
        input$incid_dtx_parameter == "Total level" ~ "Total level (in MWK)"
      )
