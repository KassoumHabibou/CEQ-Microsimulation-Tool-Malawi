  ...
  tagList(
    bslib::layout_sidebar(
      ...
      sidebar = sidebar(
        ...
        accordion(
          open = c("ineq_line_filter_panel"),
          multiple = TRUE,
          tags$h2(textOutput(outputId = ns("selected_parameter_label")),
                  style = "color: #2c3e50; font-weight: bold;"),
          accordion_panel(
            value = "ineq_line_filter_panel",
            div(
              id = ns("ineq_line_wrapper"),
              layout_columns(
                radioButtons(inputId = ns("parameter_filter"),
                             label = "Parameter to show:",
                             choices = ineq_parameter_list),
      ...
      div(
        ...
        navset_card_pill(
          ...
          nav_panel(
            "Charts",
            value = ns("ineq_chart_tab"),
            uiOutput(ns("ineq_title")),
            uiOutput(ns("ineq_caveats")),
            highchartOutput(outputId = ns("ineq_chart")) %>%
              withSpinner() %>%
          ...
          nav_panel(
            "Data",
            value = ns("ineq_data_tab"),
            reactableOutput(ns("ineq_table"))
          ),
          ...
          footer = card_footer(
            ...
            div(id = ns("ineq_download_chart"),
                download_chart_mod_ui(ns("download_ineq_chart"))),
            div(id = ns("ineq_download_data"),
                download_data_btns_ui(ns("download_ineq_data")))
          )
        )
      )
    ),
